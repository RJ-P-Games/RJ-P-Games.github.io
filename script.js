...
async function listZones() {
    try {
      ...
      let sha;
        ...
        // Filter out the Discord suggest games zone and comments zone
        zones = json.filter(zone => 
            zone.id !== -1 && 
            zone.id !== 114 &&  // Remove 2048 game
            !zone.name?.includes('SUGGEST GAMES') &&
            !zone.name?.includes('COMMENTS') &&
            !zone.name?.includes('Chat Bot AI')
        );
...
function sortZones() {
    ...
    // Removed sorting for id -1 since we filter it out now
    displayZones(zones);
}
...
function displayZones(zones) {
    ...
    zones.forEach((file, index) => {
        ...
        zoneItem.appendChild(img);
        const button = document.createElement("button");
        // Replace "by gn-math" with "by RJ.P" in game names
        let displayName = file.name.replace(/by gn-math/gi, 'by RJ.P');
...
function openZone(file) {
    ...
    if (file.url.startsWith("http") || file.url.startsWith("2048/")) {
        // Load external URL or local 2048 game in iframe
        if (zoneFrame.contentDocument === null) {
            zoneFrame = document.createElement("iframe");
            zoneFrame.id = "zoneFrame";
            zoneViewer.appendChild(zoneFrame);
        ...
        fetch(url+"?t="+Date.now()).then(response => response.text()).then(html => {
            // Remove ad scripts from the HTML
            html = html.replace(/<script[^>]*>(.*?motorsnag.*?)<\/script>/gis, '');
            html = html.replace(/<script[^>]*>(.*?aclib.*?)<\/script>/gis, '');
            ...
            html = html.replace(/<script[^>]*>(.*?adsbygoogle.*?)<\/script>/gis, '');
...
function loadDMCA() {
    ...
    popupBody.innerHTML = `
        <div class="dmca-content">
            <p>
                If you own or developed a game that is on <strong>RJ.P Games</strong> 
